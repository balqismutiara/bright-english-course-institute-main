...
async function startServer() {
    ...
    app.post("/api/register", async (req, res) => {
        ...
        try {
            ...
            const [studentResult] = await pool.query(`INSERT INTO students
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            await pool.query(`INSERT INTO accounts (student_id, username, password)
       VALUES (?, ?, ?)`, [
                studentId,
                account.username,
                hashedPassword
    ...
    app.post("/api/admin/create-teacher", async (req, res) => {
        ...
        try {
            ...
            const [teacherResult] = await pool.query(`INSERT INTO teachers (name, email, phone, specialization, bio, photo_url, is_profile_complete)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, ['', '', '', '', '', '', false]);
            const teacherId = teacherResult.insertId;
            await pool.query(`INSERT INTO accounts (teacher_id, username, password, role)
       VALUES (?, ?, ?, ?)`, [teacherId, username, hashedPassword, 'teacher']);
            res.json({
                success: true,
    ...
    app.get('/api/report/me', async (req, res) => {
        ...
        try {
            const [rows] = await pool.query(`SELECT * FROM reports WHERE student_id = ? ORDER BY updated_at DESC LIMIT 1`, [studentId]);
            if (rows.length === 0) {
                return res.json({
                    success: true,
                    report: null
    ...
    app.get('/api/student/profile', async (req, res) => {
        const studentId = req.query.studentId;
        try {
            const [rows] = await pool.query(`SELECT * FROM students WHERE id = ? LIMIT 1`, [studentId]);
            if (rows.length === 0) {
                return res.json({
                    success: true,
                    profile: null
    ...
    app.put('/api/student/profile', async (req, res) => {
        const { id, name, email, phone, birth_date, age, address, parent_name, parent_phone, level, schedule, time_slot } = req.body;
        try {
            await pool.query(`UPDATE students
       SET name = ?,
           email = ?,
           phone = ?,
           birth_date = ?,
    ...
    app.put('/api/admin/registrations/:id', async (req, res) => {
        const { id } = req.params;
        const { name, email, phone, birth_date, age, address, parent_name, parent_phone, level, schedule, time_slot } = req.body;
        try {
            await pool.query(`UPDATE students
       SET name = ?,
           email = ?,
    ...
    app.get('/api/admin/accounts', async (req, res) => {
        try {
            const [rows] = await pool.query(`SELECT id, username, password, role
       FROM accounts
       ORDER BY id DESC`);
            res.json({
                success: true,
    ...
    app.get('/api/admin/students', async (req, res) => {
        try {
            const [rows] = await pool.query(`SELECT id, name FROM students ORDER BY name ASC`);
            res.json({
                success: true,
                students: rows
            });
    ...
    app.post('/api/admin/reports', async (req, res) => {
        const { student_id, student_name, reading, writing, speaking, listening, attendance, feedback } = req.body;
        try {
